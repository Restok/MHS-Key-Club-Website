			...
			while($row = $result->fetch_assoc()){
				...
				if($result2 =  $conn->query($sql)){
					...
					while($row2 = $result2-> fetch_assoc()){
						echo "<tr><th scope = 'row'>". $row2["id"] . "</th><th scope = 'row'>". $row2["fname"]. "</th><th scope='row'>". $row2['lname']. "</th><th scope='col'><input type = 'number' name = '" .$row2['id']. $currentTableName. "' class = 'hours$currentTableName'></th><th scope='col'><input type = 'checkbox' name = 'check".(string)$currentTableName.$row2["id"]."' class = 'checkme$currentTableName'></th></tr>";
